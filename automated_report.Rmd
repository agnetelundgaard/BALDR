...
targetoverview_data <- my_hits_bmk %>% 
  ...
  mutate(across(everything(), str_replace_na),
         across(everything(), ~str_replace(.,"^NA$|^NaN$|^-$", "&mdash;"))) %>% 
  arrange(target)
  ...
  mutate(prep_format = map(table_data, ~.x %>% 
         ...
         prep_style = map(prep_format, ~.x %>% 
                            mutate(.,
                                   ...
                                   Rank = ifelse(str_detect(Rank, "mdash"), "&mdash;", Rank),
  ...
  mutate(prep_format = ifelse(n_edges != 0,
                              map(table, ~.x %>% 
                                    ...
                                    mutate(across(-c(diabetes_perc), str_replace_na),
                                           across(-c(diabetes_perc), ~str_replace(.,"^NA$|^NaN$|^-$", "&mdash;"))) %>% 
                                    select(
                                      `Target` = myhit_name,
                              ...
                              map(table, ~.x %>%
                                    ...
                                    mutate(across(-c(Target), ~str_replace_na(.) %>% str_replace(".{1,}", "&mdash;")),
                                           confint_cutoff = 1
                                           )
                                  )
                              ),
