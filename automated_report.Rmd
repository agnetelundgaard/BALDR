...
author: 
  name: "By Agnete Troen Lundgaard, Karina Banasik & Søren Brunak"
  affiliation: "Translational Disease Systems Biologi, Center for Protein Research, University Copenhagen" 
date: "Version 0.7, August 2022"
output: 
  html_document: 
    theme: "journal"
    code_folding: hide
...
<img src="./report_input/RHAPSODY_Logo_WEB_Color.png" style="position:absolute;top:50px;right:30px;with:80px;height:80px" />

<img src="./report_input/cpr-logo-vector.jpg" style="position:absolute;top:140px;right:30px;with:80px;height:80px" />
...
rmarkdown::find_pandoc(version = '2.2.1') 
```

```{r chunk setup, include= FALSE, eval = TRUE}
knitr::opts_chunk$set(warning = FALSE, 
                      ...
                      fig.retina = 3,
                      fig.path = "plots/",
                      dev = c("png", "cairo_pdf"), #png option *HAS* to be first, otherwise the html-doc will render without figures. cairo_pdf is used render with unicode symbols.
                      optipng = "-o7"
                      )
...
```{r local_pathes, results = 'hide', eval = run_on_server, echo = FALSE}
# To uncomment when back on server
# set the local pathes
const_data_path <- "/home/local/rhapsody/web_structure/data/biomarkers_1/dataset_1/"
rdata_file_1 <- paste0(params$rdata_file, '_1.RData')
rdata_file_2 <- paste0(params$rdata_file, '_2.RData')
load(paste0(const_data_path, "all_data_prioritization_report.RData"))
# Load the R objects
load(rdata_file_1)
load(rdata_file_2)
```

<!-- # The formalities {.unnumbered .tabset .tabset-fade .tabset-pills} -->

<!-- ## Rights and permissions {.unnumbered} -->

<!-- **Code ownership**: This report was written by Agnete Troen Lundgaard with supervision by Karina Banasik and Søren Brunak ("authors") of the Novo Nordisk Foundation Center for Protein Research.\   -->
<!-- \newline -->

<!-- **Funding**: This work has been funded by the IMI-RHAPSODY project (Grant agreement 115881) and the Novo Nordisk Foundation (Grant agreement NNF14CC0001) -->

<!-- **How to cite**: Please cite our paper\   -->

<!-- Informed prioritization of biomarker candidates for Type 2 Diabetes Mellitus using BALDR -->
<!-- *Agnete T. Lundgaard, Frederic Burdet, Troels Siggaard, David Westergaard, Danai Vagiaki, Lisa Cantwell, Timo Röder, Dorte Vistisen, Thomas Sparsø, Giuseppe N. Giordano, Mark Ibberson, Karina Banasik, Søren Brunak*\   -->
<!-- Submitted 2022 -->
...
<!-- If you have any questions regarding the report, please contact us on [agnete.lundgaard\@cpr.ku.dk](mailto:agnete.lundgaard@cpr.ku.dk) or  -->
<!-- [karina.banasik\@cpr.ku.dk](mailto:karina.banasik@cpr.ku.dk).\ -->
...
# Local directory containing code/data
data_dir <- here::here("data/")
source(here::here("custom_settings.R")) # Troels' setting
```
...
my_hits_exp <- my_protein_list %>% 
  ...
  mutate(omics_type = factor(omics_type, c("Transcriptomics", "Proteomics")))
...
my_hits_bmk <- my_protein_list %>% 
  ...
  left_join(., meta_exp_file %>% rename(meta_rank = rank, uniprot_gene = gene_name)) %>% 
...
table_sizes <- list(
  ...
  h_table_small_meta = if_else(num_targets_total <= 30, str_c(num_targets_total * 10 + 200, "px"), str_c(500, "px"))
...
volcano_significant_all %>% 
  ...
  scale_shape_manual(breaks = c("Transcriptomics", "Proteomics"),
...
volcano_significant %>%
  ...
  facet_wrap(~factor(omics_type, c("Transcriptomics", "Proteomics")), 
             scales = "free", 
             ncol = 2) +
  theme(legend.position = "top",
        legend.box = "horizontal",
...
meta_analysis_table$kbl_table[[1]] %>%
  scroll_box(height = table_sizes$h_table_small_meta,
             width = "100%"
             )
```
...
1. Meta-analysis of p-values from all transcriptomics and proteomics experiments within the RHAPSODY consortium (see the individual results above). The meta-analysis use the sample size-based meta-analysis described by [Willer et al, 2010](https://pubmed.ncbi.nlm.nih.gov/20616382/). The reported meta p-values are FDR-adjusted.\
...
-   In-house text-mining \
    -   UniProt AC - all diseases co-mentions (PubMed): Sum of diabetes
        and all other diseases co-mentions in PubMed abstracts. \
    -   UniProt AC - all diseases co-mentions (full-text): Sum of
        diabetes and all other diseases co-mentions in 15 mio full-text
        articles. \
...
interaction_partner_table <- ppi_table %>% 
  mutate(prep_format = map(table, ~.x %>% 
         ...
         prep_style = map2(prep_format, table, ~.x %>% 
                             mutate(., 
...
ppi_sum_table$kbl_table[[1]] %>%
  scroll_box(width = "100%", height = table_sizes$h_table_small_interpartners)
```
...
run_WGR_ppi <- my_hits_bmk %>%
  ...
  mutate(ppi_enrichResult_prep = map(data, ~.x %>% filter(!is.na(FDR))),
         ...
         ppi_figureheight = map_dbl(nrow, ~ifelse(.x == 0, 
                                                  1.5, 
                                                  ceiling(.x * 0.3) + 1)
                                    ),
...
run_WGR_shared <- ORA_features %>% 
  ...
  group_by(database) %>%
  mutate(.,
         shared_enrichResult = map2(data, ref_file, ~WGR_run_shared(interest_data = .x, ref_data = .y, database = database)),
...
-   **ATC codes**: Associated ATC code(s). For further information on the ATC system, see the WHO Collaborating Centre for Drug Statistics' [website](https://www.whocc.no/atc_ddd_index/).\   
    Colored by anatomical main group using color scheme from [Aguayo-Orozco, A., Haue, A.D., Jørgensen, I.F. et al. npj Digit 2021](https://doi.org/10.1038/s41746-021-00522-4)  

[image](report_input/ATC colors - legend figure.001.jpeg)
