...
<img src="./report_input/cpr-logo-vector.jpg" style="position:absolute;top:140px;right:-85px;width:125px" />
<img src="./report_input/IMI_Logo_Web.png" style="position:absolute;top:230px;right:-220px;width:260px" />

<img src="./report_input/RHAPSODY_Monogram_POSTER_Color.png" style="position:absolute;top:320px;right:-65px;width:80px" />
<img src="./report_input/EFPIA_Logo_Word.png" style="position:absolute;top:320px;right:-220px;width:125px" />

<img src="./report_input/WBF_SBFI_E_CMYK_pos_hoch.jpg" style="position:absolute;top:410px;right:-220px;width:125px" />
<img src="./report_input/flag_yellow_high.jpg" style="position:absolute;top:410px;right:-85px;width:125px" />
...
# example for paper
my_list <- scan(str_c(data_dir, "example_data/", "Slieker_Abbasi_candidates_uniprot.csv"), sep=',', what = "", quiet = TRUE) %>%
  as_tibble() %>%
...
targetoverview_data <- my_hits_bmk %>% 
  ...
  mutate(across(everything(), str_replace_na),
         across(everything(), ~str_replace(.,"^NA$|^NaN$|^-$", "&ndash;"))) %>% 
  arrange(target)
...
en-dash (-) indicates that no information was available for the target.
  ...
  mutate(prep_format = map(table_data, ~.x %>% 
         ...
         prep_style = map(prep_format, ~.x %>% 
                            mutate(.,
                                   ...
                                   Rank = ifelse(str_detect(Rank, "ndash"), "&ndash;", Rank),
  ...
  mutate(prep_format = ifelse(n_edges != 0,
                              map(table, ~.x %>% 
                                    ...
                                    mutate(across(-c(diabetes_perc), str_replace_na),
                                           across(-c(diabetes_perc), ~str_replace(.,"^NA$|^NaN$|^-$", "&ndash;"))) %>% 
                                    select(
                                      `Target` = myhit_name,
